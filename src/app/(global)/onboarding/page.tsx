...
function useLanguagesState() {
  ...
  const addLanguageOption = React.useCallback((language: string) => {
    setLanguageOptions((prev) => {
      if (prev.some((l) => l.toLowerCase() === language.toLowerCase())) return prev
      return [language, ...prev]
    })
  }, [])

  return { languageOptions, languageQuery, setLanguageQuery, addLanguageOption }
}
...
function useToolsState() {
  ...
  const addToolOption = React.useCallback((tool: string) => {
    setToolOptions((prev) => {
      if (prev.some((t) => t.toLowerCase() === tool.toLowerCase())) return prev
      return [tool, ...prev]
    })
  }, [])

  return { toolOptions, toolQuery, setToolQuery, addToolOption }
}
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-6">
        <Field data-slot="onboarding-avatar" name="image" invalid={!!form.formState.errors.image}>
          ...
          <FieldDescription>A photo or image that represents you across the platform. We&apos;ll use your OAuth photo by default.</FieldDescription>
  ...
  return (
    <Card>
      ...
      <CardContent className="flex flex-col gap-6">
        ...
        <Field data-slot="onboarding-languages" name="languages" invalid={!!form.formState.errors.languages}>
          <FieldLabel>Languages</FieldLabel>
          <FieldDescription>Pick from suggestions or press Enter to add a custom language.</FieldDescription>

          <FormField<OnboardingValues, "languages">
            name="languages"
            render={({ id, field }) => {
...
function LinksSection({
  ...
  form: ReturnType<typeof useForm<OnboardingValues>>
  links: ReturnType<typeof useFieldArray<OnboardingValues, "links">>
}) {
  return (
    <Card>
      <CardHeader>
...
export default function OnboardingPage() {
  ...
  async function onSubmit(values: OnboardingValues) {
    ...
    const payload = {
      ...
      tools: normalizeStringArray(values.tools),
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col mt-24 gap-6 pb-40">
      <Form form={form} onSubmit={onSubmit}>
        <PageHeader
          ...
          description="Pick a handle and fill in a few profile details. You can change everything later."
          actions={
            <FormActions className="sm:align-center sm:justify-end">
              <Button type="submit" disabled={form.formState.isSubmitting}>
                {form.formState.isSubmitting ? "Saving…" : "Join"}
              </Button>
            </FormActions>
          }
        />
